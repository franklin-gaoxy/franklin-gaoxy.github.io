原文地址: https://github.com/container-storage-interface/spec/blob/master/spec.md
...
> 关键词"unspecified"、"undefined"和"implementation-defined"应按照[rationale for the C99 standard](http://www.open-std.org/jtc1/sc22/wg14/www/C99RationaleV5.10.pdf#page=18)中的描述进行解释

An implementation is not compliant if it fails to satisfy one or more of the MUST, REQUIRED, or SHALL requirements for the protocols it implements.

> 如果某个实现无法满足其所实现的协议的“必须”、“必需”或“应”的一项或多项要求，则该实现不合规。

An implementation is compliant if it satisfies all the MUST, REQUIRED, and SHALL requirements for the protocols it implements.

> 如果一个实现满足其实现的协议的所有“必须”、“必需”和“应”要求，则该实现是合规的。

## Terminology 术语

| Term              | Definition                                                                                                           |
| ----------------- | -------------------------------------------------------------------------------------------------------------------- |
| Volume            | 将通过 CSI 在共同管理的容器内部提供的存储单元。                         |
| Block Volume      | 将在容器内显示为块设备的卷。                                                    |
| Mounted Volume    | 将使用指定的文件系统安装并显示为容器内的目录的卷。        |
| CO                | 容器编排系统，使用 CSI 服务 RPC 与插件进行通信。                                    |
| SP                | Storage Provider，CSI 插件实现的供应商。                                                         |
| RPC               | [Remote Procedure Call](https://en.wikipedia.org/wiki/Remote_procedure_call).                                        |
| Node              | 将运行用户工作负载的主机，从插件的角度可以通过节点 ID 进行唯一标识 |
| Plugin            | 又名“插件实现”，是实现 CSI 服务的 gRPC 端点。                                       |
| Plugin Supervisor | 管理插件生命周期的进程可能是 CO。                                                       |
| Workload          | 由 CO 调度的“工作”的原子单元。这可以是一个容器或容器的集合。                 |
...
> 定义行业标准“容器存储接口”（CSI），使存储供应商（SP）能够开发一次插件并使其在多个容器编排（CO）系统中工作。

### Goals in MVP 目标在MVP中?

The Container Storage Interface (CSI) will

> 这个CSI将要

* 使 SP 作者能够编写一个符合 CSI 规范的插件，该插件可以在所有实现 CSI 的 CO 中“正常工作”。
* 定义 API (RPC) 以实现：
  * 卷的动态配置和取消配置。
  * 从节点附加或分离卷。
  * 从节点安装/卸载卷。
  * 块卷和可安装卷的消耗。
  * 本地存储提供程序（例如设备映射器、lvm）。
  * 创建和删除快照（快照的来源是卷）。
  * 从快照配置新卷（恢复快照，即原始卷中的数据被擦除并替换为快照中的数据，超出了范围）。
* 定义插件 protocol RECOMMENDATIONS.
  * 描述主管配置插件的过程。
  * 容器部署注意事项(`CAP_SYS_ADMIN`, mount namespace, etc.).
...
* Misconfigured plugin sohttps://github.com/container-storage-interface/spec/blob/master/spec.mdftware SHOULD fail-fast with an OS-appropriate error code.
